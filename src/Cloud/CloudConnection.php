...
final class CloudConnection {
    ...
    /** Cloud'da bilinen/güncel değerleri döndürür. */
    public function variables(bool $refresh=true): array { if($refresh){try{$this->fetchRemoteValues(100);}catch(\Throwable){}} return $this->all(); }
    public function setMany(array $values,bool $verify=false): array { $out=[];foreach($values as $name=>$value)$out[$name]=$verify?$this->setVerified((string)$name,$value):($this->set((string)$name,$value) ? ['sent'=>true] : ['sent'=>false]);return $out; }
    public function waitUntil(string $name,callable $predicate,float $timeoutSeconds=10.0): ?string { $deadline=microtime(true)+max(.1,$timeoutSeconds);while(microtime(true)<$deadline){$v=$this->getRemote($name,min(1.0,max(.1,$deadline-microtime(true))));if($predicate($v))return $v;usleep(150000);}return null; }
    public function history(?string $name=null,int $limit=100,int $offset=0): array { $limit=max(1,min(100,$limit));$url='https://clouddata.scratch.mit.edu/logs?projectid='.rawurlencode($this->projectId).'&limit='.$limit.'&offset='.max(0,$offset);$ch=curl_init($url);curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_TIMEOUT=>12,CURLOPT_CONNECTTIMEOUT=>8,CURLOPT_USERAGENT=>'ScratchByPHP/0.8.5']);$body=curl_exec($ch);if($body===false){$e=curl_error($ch);curl_close($ch);throw new CloudException('Cloud history alınamadı: '.$e);} $status=(int)curl_getinfo($ch,CURLINFO_RESPONSE_CODE);curl_close($ch);if($status<200||$status>=300)throw new CloudException('Cloud history HTTP '.$status);$rows=json_decode((string)$body,true)?:[];$needle=$name===null?null:$this->normalizeName($name);if($needle!==null)$rows=array_values(array_filter($rows,fn($r)=>(string)($r['name']??'')===$needle));return array_map(fn($r)=>\ScratchByPHP\DTO\CloudChange::fromArray($r),$rows); }
