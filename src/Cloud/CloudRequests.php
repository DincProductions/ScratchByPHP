...
final class CloudRequests {
    ...
    public function on(string $method, callable $handler): self { $this->handlers[$method]=$handler; return $this; }
    public function handleOnce(float $timeout=10.0): ?array {
        ...
        if (!isset($this->handlers[$method])) $response=['id'=>$id,'ok'=>false,'error'=>'method_not_found'];
            ...
            try { $data=($this->handlers[$method])($params,$payload); $response=['id'=>$id,'ok'=>true,'data'=>$data]; }
