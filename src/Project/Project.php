...
use ScratchByPHP\Session; use ScratchByPHP\Cache\CacheInterface; use ScratchByPHP\Config; use ScratchByPHP\Http\HttpClient; use ScratchByPHP\Cloud\CloudConnection; use ScratchByPHP\Exceptions\ApiException; use ScratchByPHP\Analysis\ProjectAnalyzer;
final class Project {
    public function __construct(private string $id,private ?Session $session=null,private ?HttpClient $client=null,private ?CacheInterface $cache=null,private ?Config $config=null){}
    private function http():HttpClient{return $this->client??$this->session?->http()??new HttpClient(['Accept'=>'application/json'],null,$this->config);} private function rs():Session{if(!$this->session)throw new \LogicException('Authenticated Session gerekir.');return $this->session;}
    ...
    public function get():array{$key='project:'.$this->id;if($this->cache&&$this->cache->has($key))return $this->cache->get($key);$d=$this->http()->get('https://api.scratch.mit.edu/projects/'.rawurlencode($this->id))->json();$this->cache?->set($key,$d,(int)($this->config?->get('cache_ttl_project',$this->config?->get('cache_ttl',60)??60)??60));return $d;} public function refresh():array{$this->cache?->delete('project:'.$this->id);return $this->get();} public function clearCache():self{$this->cache?->delete('project:'.$this->id);return $this;} public function id():string{return $this->id;} public function title():?string{return $this->get()['title']??null;} public function author():?string{return $this->get()['author']['username']??null;} public function views():int{return(int)($this->get()['stats']['views']??0);} public function loves():int{return(int)($this->get()['stats']['loves']??0);} public function favorites():int{return(int)($this->get()['stats']['favorites']??0);}
    ...
    public function downloadProjectJson(string $path):string{$data=$this->rawJson();if(file_put_contents($path,$data,LOCK_EX)===false)throw new ApiException('Project JSON yazılamadı.');return $path;}
    public function downloadSb3(string $path):string{if(!class_exists('ZipArchive'))throw new ApiException('Gerçek SB3 paketi oluşturmak için ext-zip gerekli.');if(str_contains($path,"\0"))throw new ApiException('Geçersiz dosya yolu.');$dir=dirname($path);if(!is_dir($dir)||!is_writable($dir))throw new ApiException('Hedef klasör yazılabilir değil.');$json=$this->rawJson();$data=json_decode($json,true);if(!is_array($data))throw new ApiException('Project JSON geçersiz.');$zip=new \ZipArchive();if($zip->open($path,\ZipArchive::CREATE|\ZipArchive::OVERWRITE)!==true)throw new ApiException('SB3 ZIP oluşturulamadı.');$zip->addFromString('project.json',$json);$assets=[];foreach($data['targets']??[] as $target){foreach(array_merge($target['costumes']??[],$target['sounds']??[]) as $asset){$md5=(string)($asset['md5ext']??'');if($md5!=='')$assets[$md5]=true;}}foreach(array_keys($assets) as $md5){$url='https://assets.scratch.mit.edu/internalapi/asset/'.rawurlencode($md5).'/get/';$r=$this->http()->get($url);if($r->status>=200&&$r->status<300)$zip->addFromString($md5,$r->body);} $zip->close();return $path;}
    ...
    public function statsDto(): \ScratchByPHP\DTO\ProjectStats { return \ScratchByPHP\DTO\ProjectStats::fromArray($this->get()['stats']??[]); }
    public function commentsCollection(int $limit=20,int $offset=0): \ScratchByPHP\Collections\Collection { return new \ScratchByPHP\Collections\Collection(array_map(fn($r)=>\ScratchByPHP\Comment\Comment::fromArray($r,$this->session,'project',$this->id),$this->comments($limit,$offset))); }
    public function commentsPaginator(): \ScratchByPHP\Pagination\Paginator { return new \ScratchByPHP\Pagination\Paginator(fn($l,$o)=>$this->comments($l,$o),fn($r)=>\ScratchByPHP\Comment\Comment::fromArray($r,$this->session,'project',$this->id)); }
    public function sb3(string $path): \ScratchByPHP\Sb3\Sb3Archive { return \ScratchByPHP\Sb3\Sb3Archive::open($path); }
